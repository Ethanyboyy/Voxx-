...
| `/api/hypotheses`, `/api/ideas`, `/api/decisions`, `/api/observations`, `/api/tools`, `/api/cognition` | ORPHANED (needs verification) | No caller found via static string search. Before deleting: confirm no dynamic route construction (e.g. template-literal fetch calls) reaches them, then either wire them to real UI or remove them — dead API surface is an audit/security liability (undocumented attack surface) as well as maintenance debt. |
